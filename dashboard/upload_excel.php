  ...
  <?php if (isset($_SESSION['flash_error'])) {
   ...
   <div class="text-center mb-1">
    <?php if (empty($Admit_Cards)): ?>
      <span class="text-red-500"> Excel File not uploaded</span>
      <?php else: ?>
      <span class="text-green-700">upload Excel File uploaded : <b><?php echo $get_count['toatal_records']; ?> </b> records</span><?php endif; ?>
   </div>
    <!-- <h2 class="text-2xl font-bold text-center">Upload CSV File</h2> -->
    <form action="process_excel.php" method="post" enctype="multipart/form-data" class="bg-white p-6  rounded-lg border border-gray-300 shadow-md text-sm w-1/2 mx-auto">
    <div class="mb-4">
   
 
        <label for="template_images" class="block text-gray-700 text-sm font-bold mb-2">Upload .xlsx File </label>
          <input type="hidden" name="project_id" value="<?= $projectIdValue ?>">
          
          
        <input type="file" name="excel_file" multiple class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" accept=".xlsx" required>
