...
// ");
$recordsQuery = $conn->prepare("
    SELECT acr.id, acr.first_name, acr.registration_number,acr.roll_number, acr.project_id,acr.is_admit_card_live,
...
if ($is_live) {
  ...
  $roll_number = $records[0]['roll_number'] ?? '';
}
...
<footer class="text-center py-3 mt-5 bg-light border-top">
  <p class="mb-0">&copy; <?= date('Y') ?> <?= htmlspecialchars($project_name) ?>. All rights reserved.</p>
</footer>
