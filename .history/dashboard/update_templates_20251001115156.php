...
foreach ($column_names as $template_id => $column) {
    ...
    if (isset($_FILES['template_image']['tmp_name'][$template_id]) && !empty($_FILES['template_image']['tmp_name'][$template_id][0])) {
        ...
        $stmt_fetch_current_column_templates = $conn->prepare("SELECT id, template_image_path,template_name,page_order,template_width,template_height FROM project_templates WHERE project_id = ? AND columns_name = ? ORDER BY page_order ASC");
        ...
        foreach ($uploaded_files as $i => $tmpName) {
            ...
            if (!empty($existing_templates_for_column[$i])) {
                echo "<pre>";
                print_r($existing_templates_for_column[$i]);
                die;
                $old_template_data = $existing_templates_for_column[$i];
                $id_to_update = $old_template_data['id'];
                $insert_old_template = $conn->prepare("INSERT INTO project_templates_edited (id,project_id, template_name, template_image_path,columns_name, page_order, template_width, template_height,backup_at,updated_by,ip_address) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $insert_old_template->bind_param("iissiiissss",$id_to_update, $project_id, $old_template_data['template_name'], $old_template_data['template_image_path'], $old_template_data['columns_name'], $old_template_data['page_order'], $old_template_data['template_width'], $old_template_data['template_height'],$backup_at,$updated_by,$ip_address);
