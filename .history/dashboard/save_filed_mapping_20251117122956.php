...
// $insertBackup->close();
    ...
    foreach ($columns as $index => $column_name) {
        ...
        $field_id = $field_ids[$template_id][$index] ?? null;
       // $barcorde_base_name = $barcode_based[$template_id][$index] ?? null;
 $barcorde_base_name = $barcode_based[$template_id][$field_id] ?? null;
 echo "<pre>";
print_r($barcorde_base_name);
echo "</pre>";
exit;
            if ($column_name != 'bar_code') {
                $barcorde_base_name = null;
            }
    ...
    $stmt = $conn->prepare("UPDATE field_mappings 
        SET column_name=?,barcode_based_value = ?, x_position=?, y_position=?, width=?, height=?, cell_type=?, font_size=?, font_type=?, font_color=?, font_style=? ,line_break = ?, updated_at = ?, updated_by = ?
        WHERE id=?");

   $stmt->bind_param(
    "ssiiiisssssssis",  // ✅ Now has 14 characters
    $column_name,$barcorde_base_name, $x, $y, $w, $h, $cell, $font_size, $font_type,
    ...
    logUserAction($conn, $_SESSION['user_name'], $_SESSION['user_role'], $project_id, $template_id, $action);
                ...
                $createdAt = date('Y-m-d H:i:s');
                            ...
                            $stmt = $conn->prepare("INSERT INTO field_mappings 
                    (project_id, template_id, column_based, column_name,barcode_based_value, x_position, y_position, width, height, cell_type, font_size, font_type, font_color, font_style,line_break,created_at,created_by) 
                    VALUES (?, ?, ?, ?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?)");
                $stmt->bind_param("iisssiiissssssiss", $project_id, $template_id, $column_filter, $column_name,$barcorde_base_name, $x, $y, $w, $h, $cell, $font_size, $font_type, $font_color, $font_style, $line_break,$createdAt,$_SESSION['user_name']);
