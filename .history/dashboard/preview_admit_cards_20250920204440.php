...
if ($filter_column === 'all') {
    ...
    $query = "SELECT a.*, p.* 
              FROM admit_card_records a
              LEFT JOIN photo_path_settings p ON a.project_id = p.project_id
              WHERE a.project_id = ? AND a.`$filter_column` = ?";
    $dataQuery = $conn->prepare($query);
    $dataQuery->bind_param("is", $project_id, $column_name);
}
...
while ($template = $templateResult->fetch_assoc()) {
  $pageNumber++;
    ...
    echo $templateImage; die;$templateImage = "/uttarpradeshpoliceandrecruitmentpromotionboard/templates/68cec493c9db5_UPDATED HCMT UP (1) (1)_page-0001.jpg";
  //  $pdf->AddPage();
   ...
   // $imagePath = "uploads/templates/" . basename($templateImage);
    ...
    $fieldQuery->bind_param("iis", $project_id, $template_id,$column_name);
    }
    $fieldQuery->execute();
