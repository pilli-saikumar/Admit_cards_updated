...
require_once("../vendor/setasign/fpdi/src/autoload.php"); // Add FPDI
...
$all_records = $dataResult->fetch_all(MYSQLI_ASSOC);
...
foreach ($all_records as $data) {
   ...
   $tempPdf = new FPDF();
    foreach ($all_templates  as $template) {
        ...
        $tempPdfContent = $tempPdf->Output('', 'S');
    
    // Import the temporary PDF pages into the main merged PDF
    $pageCount = $tempPdf->setSourceFile(new \setasign\Fpdi\PdfParser\StreamReader($tempPdfContent));
    
    for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
        $templateId = $tempPdf->importPage($pageNo);
        $size = $tempPdf->getTemplateSize($templateId);
    ...
    $file_path = $base_directory . $data['registration_number'] . '.pdf';
