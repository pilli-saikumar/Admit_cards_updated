...
if ($project_id > 0) {
    $project_name = $conn->prepare("SELECT name FROM projects WHERE id = ?");
    $project_name->bind_param("i", $project_id);
    $project_name->execute();
    $project_name_result = $project_name->get_result();
    $project_name_row = $project_name_result->fetch_assoc();
    $project_name = $project_name_row['name'] ?? 'Admit Card';
...
// Prepare CSV headers
$filename = sprintf("$project_name.csv", $project_name, date("Ymd_His"));
