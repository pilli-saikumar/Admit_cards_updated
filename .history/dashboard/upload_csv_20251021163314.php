...
if ($result) {
    $allowed_columns = ['registration_number','roll_number','first_name', 'last_name', 'dob', 'sex', 'photo_path', 'signature_path','post_applied','community','nameofposition','candi_type','domicial','degree_university','inter_university','position_codes','Qualification'];
    while ($row = $result->fetch_assoc()) {
        if(in_array($row['Field'], $allowed_columns)) {
            $columns[] = $row['Field'];
        }
    }
}
  ...
  <?php if (isset($_SESSION['flash_error'])) {
   ...
   <div class="text-center mb-1">
    ...
    <div class="mb-4">
        ...
        <?php else: ?>
        <span class="text-red-500"> CSV File already uploaded</span>
        <?php endif; ?>
