      ...
      //   uttarpradeshpoliceandrecruitmentpromotionboard\templates\68d11a4e7b140_HCMT UP - JPEG.jpg
         $imgPath = __DIR__ . '/../' . $templateImage;
            ...
            foreach ($fields as $field) {
                ...
                if($data['path_type'] === 'External Path' && ($field['column_name'] === 'photo_path' || $field['column_name'] === 'signature_path')){
             ...
             //   $ext = strtolower(pathinfo($value, PATHINFO_EXTENSION));
              ...
              //  $ext = strtolower(pathinfo($value, PATHINFO_EXTENSION));
                ...
                //         try {
                //             if (filter_var($imgPath, FILTER_VALIDATE_URL)) {
                //                 // ✅ Download the remote image temporarily
                ...
                //                 if ($imgData !== false) {
                //                     $tempFile = tempnam(sys_get_temp_dir(), 'img_') . '.' . $ext;
                ...
                //                     $pdf->SetTextColor(255, 0, 0);
                //                     $pdf->SetXY($x, $y);
                //                     $pdf->Cell($width, $height, 'Image download error', 0, 0, 'C');
                //                     $pdf->SetTextColor(0, 0, 0);
                //                 }
                //             } else {
                //                 // ✅ Local image
                ...
                if (!empty($value)) {
                  ...
                  echo $path_type;
                        ...
                        // -------------------------
                            ...
                            if ($imageExists) {
                                try {
                                    // Make sure remote file access is enabled
                                    if (!ini_get('allow_url_fopen')) {
                                        @ini_set('allow_url_fopen', 1);
                                    }
                    ...
                    // -------------------------
                    // 📝 NON-IMAGE FIELDS (text, multicell)
                    // -------------------------
                    } else {
                        if ($field['cell_type'] === 'MultiCell') {
                            $pdf->MultiCell($width, 4, strtoupper($value), 0, 1, '');
