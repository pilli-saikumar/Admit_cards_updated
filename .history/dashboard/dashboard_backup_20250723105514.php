        ...
        <tbody>
            ...
            ?>
                    <tr>
                        ...
                        <td> <a href="users.php?project_id=<?= $projectId ?>" class="btn btn-primary"> Records</a></td>
                        ...
                        <td>
                            ...
                            if ($hasTemplates): ?>
                                <!-- <a href="view_templates.php?project_id=<?= $projectId ?>" class="text-green-600 mr-2" title="View Templates">  <i class="fas fa-file-image"></i></a> -->
                            <?php endif; ?>

                            <!-- Upload CSV button if no data exists -->
                            <?php if ($status === 'Pending' && !$hasTemplates): ?>
                                <a href="upload_csv.php?project_id=<?= $projectId ?>" class=" mr-2" style="color:4338CA"> <i
                                        class="fas fa-file-csv"></i>Upload CSV</a>
                            <?php endif; ?>

                            <!-- Coordinate Mapping or Editing -->
                            <?php if ($hasTemplates): ?>
                                <?php if ($projectHasFieldMappings): ?>
                                    <!-- Uncomment if needed -->
                                    <!-- <a href="preview_admit_cards.php?project_id=<?= $projectId ?>" class="btn btn-secondary">Preview</a> -->
                                    <!-- <a href="update_coordinates.php?project_id=<?= $projectId ?>" class="btn btn-warning">Update Coordinates</a> -->
                                <?php else: ?>

                                <?php endif; ?>
                            <?php endif; ?>
                            <?php if ($hasTemplates  && ($status === "Completed")) { ?>
                                <!-- <a href="generate_admit_card.php?project_id=<?= $projectId ?>" class="btn btn-primary"> <i class="fas fa-map-marker-alt"></i>  Set Coordinates</a> -->
                                <a href="generate_admit_card.php?project_id=<?= $projectId ?>" class=" mr-2" style="color:4338CA"
                                    title="Generate Admit Card"> <i class="fas fa-map-marker-alt"></i></a>
                            ...
                            <a href="upload_csv.php?project_id=<?= $projectId ?>" class="text-blue-600 mr-2"
                                title="View Templates"> <i class="fas fa-eye"></i></a>
                        </td>
