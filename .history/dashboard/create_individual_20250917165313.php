...
if (isset($_GET['batch']) && $_GET['batch'] <= $total_batches) {
    processBatch($conn, $project_id, $filter_column, $offset, $records_per_batch,$column_name, $current_batch);
    exit();
}
...
function processBatch($conn, $project_id, $filter_column, $offset, $records_per_batch,$column_name, $current_batch) {
    // Fetch templates
    if ($filter_column === 'all') {
        ...
        $templateQuery->bind_param("is", $project_id, $column_name);
    }
    ...
    foreach ($records as $data) {
        foreach ($all_templates as $template) {
           ...
           $templateImage = "/uttarpradeshpoliceandrecruitmentpromotionboard/templates/all_1_68c8f90050213_ADMITCARDFORHCMT-176POST_page-0001.jpg";
