...
foreach ($column_names as $template_id => $column) {
    ...
    if (isset($_FILES['template_image']['tmp_name'][$template_id]) && !empty($_FILES['template_image']['tmp_name'][$template_id][0])) {
        ...
        $stmt_fetch_current_column_templates = $conn->prepare("SELECT id, template_image_path, FROM project_templates WHERE project_id = ? AND columns_name = ? ORDER BY page_order ASC");
