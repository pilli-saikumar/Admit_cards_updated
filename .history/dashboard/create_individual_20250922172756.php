...
if ($filter_column === 'all') {
    ...
    $countQuery = "SELECT COUNT(*) as total FROM admit_card_records WHERE project_id = ? AND `$filter_column` = ? LIMIT ? OFFSET ?";
    $countStmt = $conn->prepare($countQuery);
    $countStmt->bind_param("isii", $project_id, $columns_name, $records_per_batch, $offset);
}
