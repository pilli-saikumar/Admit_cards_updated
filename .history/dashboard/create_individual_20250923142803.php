...
function processBatch($conn, $project_id, $filter_column, $offset, $records_per_batch, $current_batch, $columns_name) {
    ...
    if ($filter_column === 'all') {
        $query = "SELECT a.*, p.photo_path_type as path_type,p.column_based,p.path,p.prefix,p.extension,p.photo_type FROM admit_card_records a
            ...
            foreach ($fields as $field) {
                ...
                if ($field['column_name'] === 'photo_path' || $field['column_name'] === 'signature_path') {
                  ...
                  //  $get_id = $get_id[$column_based];
                    ...
                    $value = ($data['path'] ?? '') . $get_id[$column_based] . ($data['prefix'] ?? '') . '.' . ($data['extension'] ?? '');
               ...
               $ext = strtolower(pathinfo($value, PATHINFO_EXTENSION));
                ...
                if (in_array($ext, ['jpg', 'jpeg', 'png']) && !empty($value)) {
                    ...
                    if($path_type === 'Local Path'){
                     ...
                     echo "<pre>" . print_r($imageExists, true) . "<br>";
