...
foreach ($all_rows as $row) {
    ...
    //     }
    // }
    // ✅ Improved DOB Conversion
foreach ($data as $col => $val) {
    if (in_array(strtolower($col), ['dob', 'exam_date', 'trail_date', 'dv_date']) && !empty($val)) {
        $val = trim($val);

        $formats = [
            'd-m-Y', 'd.m.Y', 'd/m/Y', 'Y-m-d', 'Y.m.d', 'Y/m/d',
            'm/d/Y', 'm-d-Y', 'n/j/Y', 'n-j-Y',  // Excel-style formats
        ...
        foreach ($formats as $format) {
            $date = DateTime::createFromFormat($format, $val);
            if ($date) {
                $converted = $date->format('Y-m-d'); // Store in MySQL format
                break;
            }
        }

        // Optional: Handle Excel numeric date (e.g., 45210)
        if (!$converted && is_numeric($val)) {
            $excelDate = intval($val);
            $baseDate = new DateTime('1899-12-30');
            $baseDate->modify("+$excelDate days");
            $converted = $baseDate->format('Y-m-d');
        }

        $data[$col] = $converted ?? null;

        // 🔍 Debugging log (optional)
        error_log("DOB converted for {$data['registration_number']}: {$val} → {$data[$col]}");
    }
}
