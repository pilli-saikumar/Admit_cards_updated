...
if (empty($_SESSION["user_name"])) {
 ...
 echo "<P style='text-align:center;color:red'>Session Expired</P>";
    exit();
}
...
while ($templateRow = $templateResult->fetch_assoc()) {
    ...
    $templateImage = $templateRow['template_image_path'];
         $registrationNumber = $data['roll_number'] ?? '';
   ...
   $photo_path_settings = $conn->query("SELECT * FROM photo_path_settings WHERE project_id = $project_id")->fetch_assoc();
    ...
    $fieldQuery = $conn->prepare("SELECT column_name,barcode_based_value x_position, y_position, width, height, cell_type, font_size, font_type, font_color, font_style,line_break
                                  FROM field_mappings 
    ...
    $bar_code = $field['column_name'] === 'bar_code' ;
        ...
        $pdf->SetXY($x, $y);
          if(!empty($bar_code) ) {
            if($field['column_name'] === 'bar_code'  && !empty( $field['column_name'] === 'bar_code' )){ 
            $bar_code_based =  $field['barcode_based_value'];  // registration_number OR roll_number
            $bar_code_number = $data[$bar_code_based] ?? '';
           }else{
            $bar_code_number = '';
           }
        ...
        if ($pageNumber === 1 && !empty($bar_code_number)) {
            $barcodeimage = "https://admitcards.iroams.com/Admit_Cards/dashboard/b/barcode.php?size=30&print=false&text=" . $bar_code_number;
