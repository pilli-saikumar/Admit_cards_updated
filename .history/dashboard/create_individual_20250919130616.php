...
if(isset($_GET['project_id']) && isset($_GET['column_based'])) {
    $project_id = $_GET['project_id'];
    $filter_column = $_GET['column_based'];
    $columns_name = $_GET['columns_name'];
} 

// $project_id = 140;
// $filter_column = 'post_applied';
// $columns_name = 'Driver';
...
showBatchInterface($total_batches, $totalRecords, $records_per_batch, $project_id,$columns_name,$filter_column,$project_id);
exit();
...
function processBatch($conn, $project_id, $filter_column, $offset, $records_per_batch, $current_batch,$columns_name) {
    ...
    foreach ($records as $data) {
        foreach ($all_templates as $template) {
            $template_id = $template['id'];
          //  $templateImage = "/uttarpradeshpoliceandrecruitmentpromotionboard/templates/68cbc907e1b6f_UPDATED HCMT UP (1)_page-0001.jpg";
...
function showBatchInterface($total_batches, $totalRecords, $records_per_batch, $project_id,$columns_name) {
    ob_end_clean();
    ?>
    <!DOCTYPE html>
    <html>
