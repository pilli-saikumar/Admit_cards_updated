...
$backupSql = "INSERT INTO admitcard_edit_data SELECT * FROM admit_card_records WHERE id = ?";
$backupStmt = $conn->prepare($backupSql);
$backupStmt->bind_param("i", $id);
$backupStmt->execute();
$backupStmt->close();
$editedUserId = $_SESSION['user_id'] ?? 0;
$ipAddress = $_SERVER['REMOTE_ADDR'] ?? '';

$conn->query("UPDATE admitcard_edit_data 
             SET edited_user_id = $editedUserId, ip_address = '$ipAddress' 
             WHERE id = $id");
// Fetch column names
