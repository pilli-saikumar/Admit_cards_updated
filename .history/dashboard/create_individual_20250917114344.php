...
foreach ($all_records as $data) {
    ...
    $single_user_pdf_content = $pdf->Output('S');
    // Immediately free the memory used by the temporary PDF
    unset($pdf);
    
    // Import the pages from the single user PDF content into the final merged PDF
    try {
        // Create a temporary stream in memory to act as a file
        $temp_stream = fopen('php://temp', 'r+');
        if (!$temp_stream) {
            throw new Exception("Could not open a temporary stream.");
        }
        
        // Write the PDF content to the temporary stream
        fwrite($temp_stream, $single_user_pdf_content);
        
        // Set the stream's position to the beginning before reading
        fseek($temp_stream, 0);
        
        // Use the stream as the source file
        $page_count = $merged_pdf->setSourceFile($temp_stream);
        ...
        // Close the temporary stream to free its resources
        fclose($temp_stream);
