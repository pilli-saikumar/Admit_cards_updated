...
// $columnsResults = $conn->query("SHOW COLUMNS FROM admit_card_records");
// $columns = [];
// while ($col = $columnsResults->fetch_assoc()) {
//     $columns[] = $col['Field'];
// }
...
$columns[] = 'backup_at';
$columns[] = 'edited_user_id';
$columns[] = 'ip_address';
$columns[] = 'admitcard_record_id';
...
$dataSql = "SELECT * FROM admit_card_records WHERE id = ?";
...
foreach ($columns as $col) {
    ...
    } elseif ($col === 'edited_user_id') {
        $values[] = $edited_user_id;
    } elseif ($col === 'ip_address') {
        $values[] = $ip_address;
    } elseif ($col === 'admitcard_record_id') {
        $values[] = $id;
    } else {
...
// 6. Prepare insert
$types = str_repeat('s', count($columns)); // all strings; you can adjust if needed
$backupSql = "INSERT INTO admitcard_edit_data ($columns_list) VALUES ($placeholders)";
$backupStmt = $conn->prepare($backupSql);
$backupStmt->bind_param($types, ...$values);
$backupStmt->execute();
$backupStmt->close();

echo "Backup inserted successfully!";
// Fetch column names
