...
// Filter out only mapped columns (ignore empty mappings)
$mapped_columns = [];
foreach ($mapping as $excel_idx => $db_col) {
    if ($db_col !== '' && $db_col !== 'id') {
        $mapped_columns[$excel_idx] = $db_col;
    }
}

if (empty($mapped_columns)) {
    $_SESSION['flash_error'] = "No columns mapped. Please map at least one column.";
    header("Location: upload_excel.php?project_id=" . urlencode($project_id));
    exit;
}

error_log("Mapped columns: " . print_r($mapped_columns, true));
...
try {
    ...
    // Set read filter to only load mapped columns
    $reader->setReadFilter(new class($mapped_columns) implements \PhpOffice\PhpSpreadsheet\Reader\IReadFilter {
        private $mappedColumns;
        
        public function __construct($mappedColumns) {
        ...
        public function readCell($columnAddress, $row, $worksheetName = '') {
            ...
            // Only read mapped columns
            $columnIndex = Coordinate::columnIndexFromString($columnAddress) - 1;
            return isset($this->mappedColumns[$columnIndex]);
        }
    });
    
    $spreadsheet = $reader->load($full_path);
    $sheet = $spreadsheet->getActiveSheet();
    error_log("Excel file loaded (only mapped columns)");
} catch (Exception $e) {
...
try {
    ...
    foreach ($sheet->getRowIterator() as $row) {
        ...
        // Build data directly from mapped columns only
        $data = [];
        $colIndex = 0;
        
        foreach ($cellIterator as $cell) {
            // Only process if this column is mapped
        ...
        // Debug first row
        if ($totalRows == 1) {
            error_log("First row mapped data: " . print_r($data, true));
        }
    ...
    $_SESSION['success_message'] = "✅ Imported: $importedCount new, Updated: $updatedCount, Skipped: $skippedCount of $totalRows rows.";
