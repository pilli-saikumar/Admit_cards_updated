...
$slugQuery = $conn->prepare("SELECT name,live_url FROM projects WHERE id = ?");
...
$canidate_settings = $conn->prepare("SELECT * FROM  candidate_login_settings WHERE project_id = ?");
$canidate_settings->bind_param("i", $project_id);
$canidate_settings->execute();
$canidate_settings_result = $canidate_settings->get_result()->fetch_assoc();
...
<div class="ml-64 ">
 ...
 <div class="text-right ">
  ...
  <?php } ?>
    ...
    <table class="table table-bordered table-responsive table-sm text-sm p-2">
        <thead>
            <tr>
                <th>S.No</th>
                <th>Column Names</th>
        ...
        <!-- <button type="submit" class="btn btn-<?= ($is_live == 1) ? 'danger' : 'primary' ?>">
            ...
            <!-- <form method="POST" action="create_individual.php">
                ...
                <button type="submit" class="btn btn-success btn-sm">Create pdf </button>
                     ...
                     <?php
// Define common local server names and IP addresses
$local_hosts = ['localhost', '127.0.0.1', '::1'];
$current_server_name = strtolower($_SERVER['SERVER_NAME'] ?? '');

// Check if the current environment is local
// This check includes 'localhost', the loopback IPv4/IPv6, and also common local network ranges.
if (in_array($current_server_name, $local_hosts) || str_starts_with($current_server_name, '192.168.') || str_starts_with($current_server_name, '172.16.') || str_starts_with($current_server_name, '10.')) {
?>
    <form method="POST" action="create_individual.php">
        <input type="hidden" name="project_id" value="<?= htmlspecialchars($project_id) ?>">
        <input type="hidden" name="column_based" value="<?= htmlspecialchars($column_based) ?>">
        <input type="hidden" name="columns_name" value="<?= htmlspecialchars($projectName) ?>">
        <button type="submit" class="btn btn-success btn-sm">Create pdf </button>
    </form>
<?php
}
// Note: The form will be completely omitted from the HTML output on a production server.
?>
                    </td>
                </tr>
            <?php endwhile; ?>
              ...
              $path_url = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://{$_SERVER['HTTP_HOST']}";
                ...
                <div class="text-center mt-3">
                    <?php if(!empty($canidate_settings_result)){ ?>
                    <button onclick="window.open('<?= $baseUrl ?>', '_blank')" class="btn btn-success btn-sm">
                        Generate Admit Card URL
                    </button>
                  
    <label class="switch">
        <input type="checkbox" id="flagshipToggle" onchange="toggleFlagship(<?= $project_id ?>)" 
               <?= ($live_url == 1) ? 'checked' : '' ?>>

        <span class="slider round"></span>
    </label>
    <p id="flagshipStatus" style="margin-top:8px;">
        <?= ($live_url == 1) ? 'Live' : 'Not Live' ?>
    </p>
</div>
                    <?php }else{
                        ?>
                        <button onclick="alert('Candidate settings not found! Please set the candidate settings before generating the URL.')" style="color: black;" class="btn btn-warning btn-sm">
                        Generate Admit Card URL
                    </button>
                    <?php } ?>
...
<script>
    function toggleFlagship(projectId) {
        
   // let isLive = document.getElementById("flagshipToggle").checked ? 1 : 0;
    const toggle = document.getElementById("flagshipToggle");
    const isLive = toggle.checked ? 1 : 0;

    // Set dynamic confirmation message
    let message = isLive
        ? "Are you sure you want to make it live?"
        : "Are you sure you want to make it unlive?";
    ...
    fetch("update_flagship.php", {
        method: "POST",
        headers: {"Content-Type": "application/x-www-form-urlencoded"},
        body: "project_id=" + projectId + "&flagship=" + isLive
    })
    .then(response => response.json())
    .then(data => {
        if (data.success) {
            document.getElementById("flagshipStatus").innerText = isLive ? "Live" : "Not Live";
        } else {
            alert("Update failed!");
        }
    });
