...
foreach ($all_records as $data) {
   ...
   $temp_pdf = new FPDF();
    foreach ($all_templates as $template) {
        ...
        $temp_pdf->AddPage();
        $temp_pdf->Image($imgPath, 0, 0, 210, 297);
        ...
        foreach ($fields as $field) {
            ...
            $temp_pdf->SetFont(ucfirst($fontType), $fontStyle, intval($field['font_size']));
            $rgb = hexToRGB($field['font_color']);
            if ($rgb) {
                $temp_pdf->SetTextColor($rgb[0], $rgb[1], $rgb[2]);
            }
            ...
            if (!empty($bar_code)) {
                ...
                if ($pageNumber === 1 && !empty($registrationNumber)) {
                    ...
                    $temp_pdf->Image($barcodeimage, $x, $y, $width, $height, 'PNG');
            ...
            if (in_array($ext, ['jpg', 'jpeg', 'png'])) {
                ...
                if (!empty($value)) {
                    ...
                    if ($imageExists) {
                        try {
                            $temp_pdf->Image($imgPath, $x, $y, $width, $height);
                        } catch (Exception $e) {
                            // If image still fails, show error message
                            $temp_pdf->SetTextColor(255, 0, 0);
                            $temp_pdf->SetXY($x, $y);
                            $temp_pdf->Cell($width, $height, 'Image Error', 0, 0, 'C');
                            $temp_pdf->SetTextColor(0, 0, 0); // Reset text color
                        }
                    } else {
                        $temp_pdf->SetTextColor(255, 0, 0);
                        $temp_pdf->SetXY($x, $y);
                        $temp_pdf->Cell($width, $height, 'Image not found', 0, 0, 'C');
                        $temp_pdf->SetTextColor(0, 0, 0); // Reset text color
                    }
                }
            } else {
                ...
                if ($field['cell_type'] === 'MultiCell') {
                    ...
                    $temp_pdf->MultiCell($width, 4, strtoupper($value), 0, 1, '');
                } else {
                    $temp_pdf->SetXY($x, $y); // Reset position before each Cell
                    $temp_pdf->Cell($width, $height, $value, 0, 0, 'L'); // Add border (1)
    ...
    $single_user_pdf_content = $temp_pdf->Output('S');
    unset($temp_pdf); // Unset the temporary PDF to free memory
    
    // Import and append the pages of the single user PDF to the main merged PDF
    try {
        $page_count = $merged_pdf->setSourceFile('data://application/pdf;base64,' . base64_encode($single_user_pdf_content));
...
// Output the final single merged PDF
ob_end_clean();
