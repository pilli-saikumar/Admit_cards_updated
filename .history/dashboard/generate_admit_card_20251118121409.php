    ...
    <?php if (!empty($uploaded_templates)) { ?> 
        ...
        <form method="POST" action="save_filed_mapping.php">
            ...
            <?php  foreach ($uploaded_templates as $template):
             ...
             // $template_path = "/uttarpradeshpoliceandrecruitmentpromotionboard/templates/68d265d1a1c66_HCMT - 23 Admit card_page-0001 (1).jpg";
            ...
            <div class="coordinateField mb-3" data-field-id="<?= $field['id'] ?>">
                ...
                <div class="row row-cols-6">
                    ...
                    <div class="col-md-2">
                        ...
                        <select name="column_name[<?= $template_id ?>][]" class="form-control column_name"  required>
                    ...
                    <!-- <div class="col barcode-options" style="display:none;">
    <label>Barcode Based:</label>
    <select name="barcode_based[<?= $template_id ?>][<?= $field['id'] ?>]" class="form-control">
        <option value="registration_number">Registration Number</option>
        <option value="roll_number">Roll Number</option>
    </select>
</div> -->
<div class="col barcode-options"
     style="<?= ($field['column_name'] == 'bar_code') ? '' : 'display:none;' ?>">
    <label>Barcode Based:</label>
    <select name="barcode_based[<?= $template_id ?>][<?= $field['id'] ?>]" class="form-control">
        <option value="registration_number"
            <?= ($field['barcode_based_value'] == 'registration_number') ? 'selected' : '' ?>>
            Registration Number
        </option>

        <option value="roll_number"
            <?= ($field['barcode_based_value'] == 'roll_number') ? 'selected' : '' ?>>
            Roll Number
        </option>
    </select>
</div>
...
<div class="col barcode-options" style="display:none;">
    <label>Barcode Based:</label>
    <select name="barcode_based[<?= $template_id ?>][__NEW_ID__]" class="form-control">
        <option value="registration_number">Registration Number</option>
        <option value="roll_number">Roll Number</option>
    </select>
</div>
    ...
    const newFieldHTML = `
        <div class="coordinateField mb-3" data-field-id="${uniqueId}">
            ...
            <div class="row row-cols-6">
                <div class="col-md-2">
                    <label>Column Name:</label>
                    <select name="column_name[${templateId}][]" class="form-control column_name" required>
                        <option value="">Select Column</option>
                        <?php foreach ($columns as $colName): ?>
                            <option value="<?= htmlspecialchars($colName) ?>"><?= htmlspecialchars($colName) ?></option>
                        <?php endforeach; ?>
                                 ...
                                 <div class="col barcode-options" style="display:none;"  >
  <label>Barcode Based:</label>
       <select name="barcode_based[${templateId}][${uniqueId}]" class="form-control">
        <option value="registration_number">Registration Number</option>
        <option value="roll_number">Roll Number</option>
...
// }
    ...
    $(document).on('change', '.column_name', function() {
        ...
        const fieldGroup = $(this).closest('.coordinateField');
        const container = $(this).closest('.coordinateFieldsContainer');
      //  const fieldId = $(this).data('field-id');
        const templateId = container.data('template-id');
        const filedid  = fieldGroup.data('field-id');
       
        if (column_name === 'bar_code') {
        fieldGroup.find('.barcode-options').show(); // only in this row
    } else {
        fieldGroup.find('.barcode-options').hide();
    }
