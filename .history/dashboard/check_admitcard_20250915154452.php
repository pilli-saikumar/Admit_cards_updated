...
// Determine if status is active *as of today*
// $currently_active = ($today >= $live_date && $today <= $unlive_date && $is_live == 1);




// Set button label
//$buttonLabel = $currently_active ? "Deactivate" : "Activate";
//$buttonClass = $currently_active ? "danger" : "primary";
    ...
    // if ($is_live == 1) {
    //     $liveCount++;
    //     $projectNamesForSlug[] = $projectName; // store for slug
    // }
        ...
        <!-- <button type="submit" class="btn btn-<?= ($is_live == 1) ? 'danger' : 'primary' ?>">
            ...
            <?php endwhile; ?>
              ...
              $path_url = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://{$_SERVER['HTTP_HOST']}";
                ...
                <?php } ?>
                 <?php <button onclick="window.open('<?= $baseUrl ?>/create_individual.php?project_id=<?= $project_id ?>', '_blank')" class="btn btn-success btn-sm">
                      Create Individual Admit Cards
                    </button> ?>
        </tbody>
