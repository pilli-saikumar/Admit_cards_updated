...
if ($result) {
    while ($row = $result->fetch_assoc()) {
        if (in_array($row['Field'], ['photo_path', 'signature_path','photo'])) {
            $columns[] = $row['Field'];
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_photo_settings'])) {
  ...
  if($photo_type === 'photo_path'){
     $folder = 'photos';
  }elseif($photo_type === 'signature_path'){
     $folder = 'signatures';
  }elseif($photo_type === 'photo'){
     $folder = 'extra_photos';
  }

    $upload_dir = dirname(__DIR__) . "/$project_slug/" . $folder . "/";
    ...
    if ($photo_path === 'Local Path') {
        ...
        foreach ($candidate_photo['tmp_name'] as $index => $tmpName) {
            ...
            $extension = pathinfo($originalName, PATHINFO_EXTENSION);
            $baseName = strtolower(pathinfo($originalName, PATHINFO_FILENAME));
           // $baseName = preg_replace('/_photo$/i', '', $baseName);
           if ($photo_type === 'photo_path') {
            $folder = 'photos';
            $suffix_to_remove = '_photo';
        } elseif ($photo_type === 'signature_path') {
            $folder = 'signatures';
            $suffix_to_remove = '_sign';
        } else {
             // Handle 'photo' (extra_photos) or other cases if needed
            $folder = 'extra_photos';
            $suffix_to_remove = ''; // Assuming extra photos don't have a specific suffix
        }

        // Remove the specific suffix based on the expected upload type
      //  $baseName_for_mapping = preg_replace("/{$suffix_to_remove}$/i", '', $baseName);
            ...
            // }
        if (!empty($photo_prefix)) {
                    $newFileName = $baseName_for_mapping . $photo_prefix . '.' . $extension;
                } else {
                    // Use the baseName_for_mapping (e.g., '2590002') for the new file name
                    // AND append the correct suffix for a clean naming convention in the folder if desired
                    // OR simply use the original baseName if the file name should contain the suffix (e.g., 2590002_photo.jpg)
                    // I'll stick close to your original intent, using $baseName for the new filename
                    $newFileName = $baseName . '.' . $extension;
                }
                    if (!empty($photo_prefix) && str_starts_with($baseName, strtolower($photo_prefix))) {
                $baseName = substr($baseName, strlen($photo_prefix));
                ...
                if (move_uploaded_file($tmpName, $destination)) {
                    ...
                    $dbPath = "$project_slug/" . $folder . "/$newFileName";
                  
 

                   $dbPath = "$project_slug/" . ($photo_type === 'photo_path' ? 'photos' : 'signatures') . "/$newFileName";
                 //  $stmt = $conn->prepare("UPDATE admit_card_records SET `$photo_type` = ?, path_type = ? WHERE registration_number = ?");
                    $stmt = $conn->prepare("UPDATE admit_card_records SET `$photo_type` = ?, path_type = ? WHERE project_id = ? AND `$photo_column_based` = ?");
                    $stmt->bind_param("ssss", $dbPath, $photo_path, $project_id, $candidateId);
                    $stmt->execute();
                    $mapped_photos++;
                }
            } else {
