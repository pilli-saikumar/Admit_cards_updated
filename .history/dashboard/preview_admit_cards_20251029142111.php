...
while ($template = $templateResult->fetch_assoc()) {
  ...
  // uttarpradeshpoliceandrecruitmentpromotionboard\templates\68d265d1a1c66_HCMT - 23 Admit card_page-0001 (1).jpg
   // $imagePath = "uploads/templates/" . basename($templateImage);
    ...
    if (
    $data['path_type'] === 'External Path' && 
    in_array($field['column_name'], ['photo_path', 'signature_path', 'photo'])
) {
    $column_based = $data['column_based'] ?? 'id';
    $get_id_query = "SELECT `$column_based` FROM admit_card_records WHERE project_id = $project_id AND id = $record_id";
    $get_id_result = $conn->query($get_id_query)->fetch_assoc();
    $get_id = $get_id_result[$column_based] ?? '';

    $extension = $data['extension'] ?? 'jpg';

    // Choose prefix and folder based on column
    ...
    $value = rtrim($data['path'], '/') . '/' . $folder . $get_id . $prefix . '.' . $extension;
