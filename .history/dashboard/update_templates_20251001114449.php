    ...
    if (isset($_FILES['template_image']['tmp_name'][$template_id]) && !empty($_FILES['template_image']['tmp_name'][$template_id][0])) {
        ...
        foreach ($uploaded_files as $i => $tmpName) {
            ...
            if (!empty($existing_templates_for_column[$i])) {
                ...
                $insert_old_template->bind_param("iiisiii", $project_id, $id_to_update, $old_template_data['template_name'], $old_template_data['template_image_path'], $old_template_data['page_order'], $old_template_data['template_width'], $old_template_data['template_height']);
