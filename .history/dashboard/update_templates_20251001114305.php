    ...
    if (isset($_FILES['template_image']['tmp_name'][$template_id]) && !empty($_FILES['template_image']['tmp_name'][$template_id][0])) {
        ...
        foreach ($uploaded_files as $i => $tmpName) {
            ...
            if (!empty($existing_templates_for_column[$i])) {
                ...
                $insert_old_template = $conn->prepare("INSERT INTO project_templates_edited (id,project_id, template_id, template_name, template_image_path, page_order, template_width, template_height) VALUES (?, ?, ?, ?, ?, ?, ?)");
