...
if (!empty($slug)) {
    ...
    if(!empty($forgot_settings)){
        $stmt = $conn->prepare("SELECT * FROM forgot_view_details WHERE project_id = ?");
        $stmt->bind_param("i", $project_id);
        $stmt->execute();
        $result = $stmt->get_result();

        while ($row = $result->fetch_assoc()) {
            $forgot_view_details[] = $row;
        }
        $stmt->close();
...
$sql_settings = "SELECT column_name, label_name FROM forget_view_details WHERE project_id = ? AND view_status = 1";
$stmt_settings = $conn->prepare($sql_settings);
if ($stmt_settings === false) {
    die("Error preparing settings statement: " . htmlspecialchars($conn->error));
}
$stmt_settings->bind_param("i", $project_id);
$stmt_settings->execute();
$forgot_settings = $stmt_settings->get_result()->fetch_all(MYSQLI_ASSOC);
$stmt_settings->close();
...
// Get the columns to select and the labels for display
foreach ($forgot_settings as $setting) {
    $selectColumns[] = $setting['column_name'];
    $columnLabels[$setting['column_name']] = $setting['label_name'];
}

// Add static columns you always want to select
$selectColumns[] = 'id';
...
// Always generate a new captcha for each page load to ensure consistency
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        // Get columns to select from forgot_view_details
        $selectColumns = ['id', 'registration_number']; // Default columns
        if (!empty($forgot_view_details)) {
            foreach ($forgot_view_details as $view_detail) {
                if (!empty($view_detail['column_name']) && !in_array($view_detail['column_name'], $selectColumns)) {
                    $selectColumns[] = $view_detail['column_name'];
                }
            }
        }
        ...
        $query = "SELECT $selectClause FROM admit_card_records WHERE $whereClauseStr AND project_id = $project_id";
...
<body>
    ...
    <div class="login-card">  
        ...
        <h3><?= $project_name ?></h3>
        <h4 class="text-center text-danger font-weight-bold"><?= $forgot_label_name ?></h4>
    ...
    <input type="text" 
        ...
        <a href="../<?= $slug ?>/index.php" class=" text-danger fw-bold">Back to Login</a>
          ...
          <?php echo "<pre>"; print_r($forgot_view_details); echo "</pre>"; ?>
         <?php if(isset($userData))  : ?>
   
         <?php if($userData){ ?>
         <div class="alert alert-success text-center mt-2">PNO Number: <?= $userData['registration_number'] ?>
        <br>
